...
class X {
  ...
  equals = K;
  constructor(t, n, i) {
    typeof t == "function" ? (this.fn = t, this._value = void 0, this.effect = n || !1, this.state = C, n && (M.push(this), F?.(this))) : (this.fn = void 0, this._value = t, this.state = O, this.effect = !1), i && (this.label = i);
  }
  get value() {
    return this.get();
  }
  ...
  stale(t) {
    if (this.state < t && (this.state === O && this.effect && (M.push(this), F?.(this)), this.state = t, this.observers))
      for (let n = 0; n < this.observers.length; n++)
        this.observers[n].stale(R);
  }
...
function Q(e, t = [], n = [], i, l) {
  ...
  for (; s < o || u < a; )
    if (o === s) {
      ...
      for (; u < a; )
        p && p[u] ? c.insertBefore(p[u], y) : p[u] = H(() => i(n[u]), e, y, l), u++;
    } else if (a === u)
      for (; s < o; )
        (!h || !h.has(t[s])) && (I(d[s].myid, l), c.removeChild(d[s])), s++;
    else if (t[s] === n[u])
      p[u] = d[s], s++, u++;
    else if (t[o - 1] === n[a - 1])
      p[a - 1] = d[o - 1], o--, a--;
      ...
      if (!x) {
        ...
        if (x = !0, !y)
          return W(l), S(e), Q(e, [], n, i, l);
      }
      if (h.has(t[s])) {
        const m = h.get(t[s]);
        if (u < m && m < a) {
          let g = s, $ = 1;
          for (; ++g < o && g < a && h.get(t[g]) === m + $; )
            $++;
          if ($ > m - u) {
            const G = d[s];
            for (; u < m; )
              p && p[u] ? c.insertBefore(p[u], G) : p[u] = H(() => i(n[u]), e, G, l), u++;
          } else
            p && p[u] ? c.replaceChild(p[u], d[s]) : p[u] = te(() => i(n[u]), d[s], l), s++, u++;
        } else
          s++;
      } else
        I(d[s].myid, l), c.removeChild(d[s++]);
    }
}
let A = [];
function ue(e) {
...
function k(e) {
  Object.getOwnPropertySymbols(e).forEach((t) => {
    e[t]?.frag && (e[t].stale = !0, k(e[t]), delete e[t]);
  });
}
function D(e, t, n) {
  const i = Symbol("$comp");
  if (n[i] = {}, e.before(document.createTextNode("")), e.after(document.createTextNode("")), n[i].frag = L(e), t._map) return t(n, i);
  P(async () => {
    if (!n[i]) return;
    if (n.stale || n[i].stale) return delete n[i];
    const l = A.length, r = E.length, c = B(n[i]);
    c.length && c.forEach((o) => typeof o == "function" && o()), k(n[i]);
    let s = t();
    if (s instanceof Promise && (s = await s), !!n[i]) {
      ...
      s = typeof s == "function" && !s?.html ? T(s) : s, ie(s, n[i]);
    }
  }, { effect: !0 });
}
function ce(e, t) {
...
function _(e, t, n = { 0: {} }) {
  ...
  if (e.nodeType && (e = L(e)), typeof t == "function" && !t.html && (t = T(t)), typeof t == "function" && t.html && (t = T(t)), t.html === void 0) return console.error("MiNi: unknown input to renderClient", t);
  ...
  return S(e), e.next ? e.next.before(r.content) : c?.appendChild(r.content), e.destroy = () => {
    const s = B(n);
    s.length && s.forEach((o) => typeof o == "function" && o()), k(n), S(e);
  }, e;
}
